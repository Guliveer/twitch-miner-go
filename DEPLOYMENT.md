...
sudo ./tools/install-service.sh status
...
```bat
REM Right-click tools\install-service.bat and select "Run as administrator"
...
```bat
tools\install-service.bat start       REM starts (triggers a rebuild first)
...
# Generate a key first: ./tools/gen-cookie-key.sh
